...
//2022.01.04
//오늘은 퀘스트 내용을 볼수있는 창과
//플레이어의 상태를 나타내주는 창을 출력하는 기능을 만들어 보려한다.
//플레이어 상태창에 나타낼 정보 목록
//1.체력(현재,최대)
//2.공격력
//3.방어력
...
//Gamewindows클래스에 StateWindow를 추가하였다.
//상태창을 나타낼 Choice를 구분하기 위해(메인화면등에서는 나오지 않아야 한다.) Choice에 IsShowStateWindow라는 이름의 변수를 추가하였다.
//그리고 Display에서 StateWindow(Player info,int x,int y)를 호출해 IsShowStateWindow가 true인 경우 상태창을 띄우려고 한다.
//Player의 정보를 상태창에 띄우는데에 성공했다.
//그런데 Player의 AttackPower는 무기를 장착할때 Weapon의 AttackPower와 따로따로 분리되어있는데
//Defense나 AttackSpeed는 장비 착용시 플레이어의 능력치에 합산되어 저장되어 있다.
//이유는 BattleSystem에서 Dodge,Block을 구현할때 플레이어의 AttackSpeed와 Defense값만 불러와서 판단하기 때문인데
//합산,분리중 통일해서 구현해야 할것 같다. 잠깐 생각해 봤을때는 분리해서 관리하는 것이 더 많은 기능을 추가할때 유리할 것 같다.

//상태창에 현재 진행중인 퀘스트와 그 퀘스트의 완료 여부를 나타나게 했다.
//오늘은 시간이 없어서 못했지만 퀘스트의 진행정도도 표시하게 하고싶다.
//내일은 퀘스트 내용을 볼수 있는 창을 만들고
...
//지금은 퀘스트나 몬스터 사냥으로 Item을 획득할때 인벤토리가 가득차면 Item획득 메세지만 뜨고 실제로는 획득 안된다. 수정요함(수정완료 2022.01.10)
